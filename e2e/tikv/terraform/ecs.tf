...
locals {
  ...
  # MUSTER_PD_SERVICE / MUSTER_TIKV_SERVICE are CloudMap discovery names, for
  # instances(). MUSTER_ECS_CLUSTER / MUSTER_ECS_SERVICE are the ECS names, for
  # all_ecs_tasks_running().
  #
  ...
  muster_env = [
    ...
    { name = "MUSTER_ECS_CLUSTER", value = aws_ecs_cluster.main.name },
  ]
}
  ...
  container_definitions = jsonencode([
    {
      ...
      entryPoint = [
        ...
        "-kv-table", aws_dynamodb_table.kv.name,
      ...
      environment = concat(local.muster_env, [
        { name = "MUSTER_ECS_SERVICE", value = local.pd_service_name },
      ])
  ...
  container_definitions = jsonencode([
    {
      ...
      environment = concat(local.muster_env, [
        { name = "MUSTER_ECS_SERVICE", value = local.tikv_service_name },
      ])
