...
# Our own ECS cluster and service, for all_ecs_tasks_running(). Left unset these
# are None and the builtin falls back to the task metadata endpoint, which muster
# reads once at startup, best-effort, with no retry. When that comes up empty the
# builtin can only raise — and the raise escapes resolve(), so the workload never
# starts at all. The deployment knows both names for certain, so it says so.
ECS_CLUSTER = env("MUSTER_ECS_CLUSTER")
ECS_SERVICE = env("MUSTER_ECS_SERVICE")
...
def all_pd_tasks_running():
    return all_ecs_tasks_running(cluster = ECS_CLUSTER, service = ECS_SERVICE)
...
def lineup():
    ...
    if not ECS_CLUSTER or not ECS_SERVICE:
        log("pd: ECS cluster/service unknown; not waiting for the line-up")
        return

    # select() waits for the poll to settle and hands back the promise itself,
    # so unlike join() it does not re-raise a rejection. all_ecs_tasks_running()
    # calls DescribeServices, which can fail for reasons that have nothing to do
    # with whether the tasks are up: throttling, a missing permission, an IAM
    # condition key that does not match. None of those should stop PD starting.
    select(poll(all_pd_tasks_running, "120s", interval = "5s"))
